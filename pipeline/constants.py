...
EXCLUDED_STATUS_SERIES_EXCEPTIONS = {"L0系", "国鉄DF50形"}
...
任何字段都不得包含URL、Markdown链接、引用来源、括号注释或中日/英日双语拼接；web_search只用于判断，不得把引用写入JSON。
- 子车型：一些较大型号车型家族下的细分型号，注意不是番台。例如，KiHa 40系列下有KiHa 40, KiHa 47, KiHa 147等车型。这一项输出请不要带任何series等后缀，也不要加上运营商前缀例如JRF。直接输出子车型的名称，若是番台区分则带完整车型，例如KiHa 147，E231-1000等。气动车型的罗马字表记固定使用KiHa。如果没有明确的子车型信息请复制其原车型，不要随意猜测，也不要直接写机车型号。
...
- 运营公司：英文与日文必须分别写入对应字段，不得在单个字段内用斜杠拼接。若提示词末尾附有从label_metadata读取的规范术语，必须使用其中的日英对应写法；未收录的运营公司使用一个稳定的正式英文名和日文名。
- 特殊编成：以特殊名称命名的一些编成，例如etSETOra，Resort Shirakami Aoike。由于文件是由英语写就，请直接输出罗马字或英文标记的名字。不要写入与车辆特殊编成无关的特急或优等列车爱称，例如'Odoriko'.
- 特殊涂装：一些车型拥有的特殊涂装，常见以"livery"在路径或文件名中出现，例如"Hokutosei livery"。统一输出日文规范名，例如北斗星、国鉄色、ピンクライン，不要带“涂装”等后缀。

术语与格式规范：
- 气动车型罗马字固定写作KiHa，不使用Kiha。
- 车型与番台组合使用连字符，例如EF65-500，不写EF65 500。
- ピンクライン、国鉄色、クリームと緑、新潟色I使用这里给出的规范写法，不输出其英文或异体写法。
