...
立即重算组，剩余不足两个成员时删除该组。人工确认前 Stage 14 默认中断：
...
- `crop_duplicate_groups`：同一原图 SHA1 内高 IoU crop 的重复组、候选标签、线性头建议与人工解析结论，并用 `exclusion_reason` 结构化记录整组 `bad_crop` / `out_of_label_space` 排除。原始 `images` / `crops` 来源行不删除；Stage 14 对已解析组只导出一个与最终标签匹配的代表成员。重复复核 UI 排除单个成员或整组时会同步写入对应 `crops.noise_review_*` 字段。
