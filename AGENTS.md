...
给出候选内建议；已人工标记为 `bad_crop` / `out_of_label_space` 的具体 crop
不参与分组。UI 可将单个成员或整组原地标记为这两类人工噪声；单成员排除后
立即重算组，剩余不足两个成员时删除该组。人工确认前 Stage 16 默认中断：
...
这是导出前按需运行的人工辅助工具，不注册为自动 pipeline stage。Stage 16
...
翻译表位于 `label_metadata_translation.review_file_path`。本阶段只导出数据库尚不存在的新 label；operator 按 crop 的当前有效 label 聚合 Stage 06 逐图识别的 `images.operator_jp` / `images.operator_en`，并用已有 `label_metadata` 术语表补齐中文名。翻译未完成时中断后续 pipeline，填写 `label_en`、`label_zh` 与三语 operator JSON 数组后重跑即可事务写入。已有 `label_metadata` 行不会被翻译表覆盖。
...
| `llm_labeling` | `stage_06_llm_metadata_labeling.py` | 用 OpenAI API 从分类路径抽取番台、子型号、运营公司等元数据；运行时从 `label_metadata` 生成 operator 日英术语表并附加到提示词，同时约束引用与输出格式；只回写 `llm_metadata_processed = 0` 的新图片，同路径已有 checkpoint 时直接复用，避免覆盖既有 metadata |
...
| `label_metadata_translation` | `stage_14_label_metadata_translation.py` | 在 Stage 13 与 crop 存储之间增量导出新 label 翻译表；按有效 label 聚合 Stage 06 逐图 operator，填写后校验并回写 `label_metadata`，不覆盖既有规范行 |
| `crop_duplicate_detection` | `stage_15_crop_duplicate_detection.py` | 在最终导出前按原图 SHA1、detector 配置与 bbox IoU 聚合同一物理 crop；同标签组自动解析，跨标签组写入 `crop_duplicate_groups` 并由已有分类 artifact 提议候选标签 |
| `store_crops` | `stage_16_store_crops.py` | 将 crop 图像保存为最终数据集，并生成 `metadata.csv` / `labels.csv`，同时从数据库 `label_metadata` 规范表导出 `l10n_metadata.json`；`metadata.manual_reviewed` 表示人工复核为 `ok` 的高确信样本 |
...
东武20000系转籍改造的アルピコ交通20100形也已进入同一 scope：人工目录先 merge 到基础 `東武20000系`，Stage 08 再按锁定 operator 拆成 `アルピコ交通20100形`。
...
- `category_tree_checkpoints`：Stage 03 已完整遍历的分类子树，按车型、根分类、category 与相对递归深度记录；用于重跑时整棵跳过，同时避免把不同车型的排除规则混用。
...
- `crop_duplicate_groups`：同一原图 SHA1 内高 IoU crop 的重复组、候选标签、线性头建议与人工解析结论，并用 `exclusion_reason` 结构化记录整组 `bad_crop` / `out_of_label_space` 排除。原始 `images` / `crops` 来源行不删除；Stage 16 对已解析组只导出一个与最终标签匹配的代表成员。重复复核 UI 排除单个成员或整组时会同步写入对应 `crops.noise_review_*` 字段。
